...
import PisuClient.mods.ModInstances;
...
public class PisuClientMovementInput extends MovementInput {
	...
	public PisuClientMovementInput(GameSettings gameSettings) {
	...
	public String getDisplayText() {
		...
		boolean isFlyng = mc.thePlayer.capabilities.isFlying;
		...
		if(isFlyng) {
			if(originalFlySpeed > 0.0F) {
				displayText = "[Flyng (" + df.format(boostedFlySpeed / originalFlySpeed) + "x Boost)] "; 
			}
			else {
				displayText = "[Flyng] ";
			}
		...
		else if(sprint && !isFlyng && !isRiding) {
			if(isHoldingSprint) {
